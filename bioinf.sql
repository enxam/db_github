...
--
-- Temporary view structure for view `genevw`
--

DROP TABLE IF EXISTS `genevw`;
/*!50001 DROP VIEW IF EXISTS `genevw`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `genevw` AS SELECT 
 1 AS `genus`,
 1 AS `species`,
 1 AS `name`,
 1 AS `acc`,
...
--
-- Final view structure for view `genevw`
--
...
/*!50001 VIEW `genevw` AS select `modorg`.`genus` AS `genus`,`modorg`.`species` AS `species`,`class`.`name` AS `name`,`gene`.`acc` AS `acc`,`gene`.`len` AS `len`,`gene`.`descr` AS `descr` from ((`gene` join `modorg` on((`gene`.`mo_id` = `modorg`.`id`))) join `class` on((`modorg`.`class_id` = `class`.`id`))) */;
...
-- Dump completed on 2019-12-04 14:57:16
